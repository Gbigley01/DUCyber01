...
On the third day of our introduction to networking, you will learn how to gather data from specific networks using footprinting.
...
- Define footprinting as a set of methods used by security professionals and hackers to determine network vulnerabilities.
...
The slides for today can be viewed on Google Drive here: [8.3 Slides](https://docs.google.com/presentation/d/1m54-wlml67sdfg3w3Zpo_M17v8G6Pl9JC1sS1ZJWJO0/edit#slide=id.g4789b2c72f_0_6)
...
- The process of gathering data for a specific network, known as **footprinting**.
...
### 02. Introduction to Footprinting
...
One prominent method used by both attackers and security professionals is **footprinting**.

- **Footprinting** is the process of gathering data for a specific network, usually for the purpose of finding ways to gain access into that network.

- Footprinting can provide the following information:
...
In today’s class, we will introduce how security professionals use protocols and tools that exist in Layers 2, 3, and 4 of the OSI model to footprint a network.
...
The goal of footprinting is to gain access to a particular network.  

An important step of footprinting is checking if an external host is actually operating and accepting connections.
...
A failed response doesn't necessarily indicate that the host is down.  It may simply mean that the host is not allowing requests from ICMP.
  ...
  - Businesses may restrict ICMP requests for security reasons, such as preventing external parties from footprinting.  
...
For multiple ping requests, we can use **fping**, a utility that can run ping requests against multiple IPs, or a range of IPs, with a single request.
 ...
 - You can install `fping` with the command `apt install fping`.
...
### 07. Footprinting with `ping`

- [Activity File: Footprinting with `ping`](Activities/07_Footprinting_with_ping/unsolved/readme.md)


### 08. Review Footprinting with `ping`

- [Solution Guide: Footprinting with `ping`](Activities/07_Footprinting_with_ping/solved/readme.md)
...
### 10. Footprinting with `traceroute`


- [Activity File: Footprinting with `traceroute`](Activities/11_Footprinting_with_traceroute/unsolved/readme.md)


### 11. Review Footprinting with `traceroute` Activity


- [Solution Guide: Footprinting with `traceroute`](Activities/11_Footprinting_with_traceroute/solved/readme.md)
...
This footprinting process of sending SYN requests to many ports on a network is called a **SYN Scan**.
